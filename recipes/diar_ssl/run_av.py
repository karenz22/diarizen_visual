...
# --- Import your AV specific components ---
# Ensure dataset_av.py is in the same directory or python path
from dataset_av import av_collate_fn
...
def run(config, resume):
    ...
    if config["finetune"]["finetune"]:
        ...
        # Ensure ckpt loading logic is compatible with AV model keys if strictly matching
    ...
    # Pass model info to dataset (needed for label alignment)
    ...
    # Define the partial collate function with the config parameter
    av_collate_fn_partial = partial(
        av_collate_fn, 
    ...
    if "train" in args.mode:
        ...
        train_dataloader = DataLoader(
            ...
            collate_fn=av_collate_fn_partial,  # <--- Use the partial function here
            shuffle=True, 
    ...
    #     train_dataloader = DataLoader(
    #         dataset=train_dataset, 
    #         collate_fn=av_collate_fn,  # Use the AV collate function
    #         shuffle=True, 
    #         **config["train_dataset"]["dataloader"]
    #     )
    #     train_dataloader = accelerator.prepare(train_dataloader)
    ...
    # 5. Instantiate AV Trainer
    # initialize=False is required because we pass objects (accelerator, model, etc.) manually
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Audio-Visual Diarization Training")
    parser.add_argument("-C", "--configuration", required=True, type=str, help="Configuration (*.toml).")
    parser.add_argument("-M", "--mode", nargs="+", type=str, default=["train"], choices=["train", "validate"], help="Mode.")
    parser.add_argument("-R", "--resume", action="store_true", help="Resume from checkpoint.")
    parser.add_argument("-FT", "--finetune", action="store_true", help="Finetuning flag.")

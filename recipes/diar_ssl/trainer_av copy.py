# from diarizen.trainer_single_opt import Trainer

# class AVTrainer(Trainer):
#     def training_step(self, batch, batch_idx):
#         inputs = batch['inputs'] # Now a dict {waveforms, visual}
#         targets = batch['ts']
        
#         # Move visual to device
#         inputs['waveforms'] = inputs['waveforms'].to(self.device)
#         inputs['visual'] = inputs['visual'].to(self.device)
#         targets = targets.to(self.device)

#         # Forward
#         predictions = self.model(inputs)
        
#         # Calculate loss (assuming segmentation task)
#         # You might need to flatten predictions/targets depending on loss function
#         loss = self.criterion(predictions, targets) 
        
#         return {"loss": loss}

#     # You would also override validation_step similarly
...
class AVTrainer(BaseTrainer):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # AV modification: Retrieve 'inputs' dict (waveforms + visual) and 'ts' targets
        inputs = batch['inputs']
        target = batch['ts'] 
        
        # Ensure nested dict tensors are on the correct device
        ...
        # powerset conversion (same as original)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        # AV modification: Retrieve 'inputs' dict (waveforms + visual)
        inputs = batch['inputs']
        target = batch['ts'] 

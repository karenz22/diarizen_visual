...
class AV_DiariZenPipeline(DiariZenPipeline):
    ...
    def get_av_segmentations(self, waveform: torch.Tensor, sample_rate: int, sess_name: str) -> SlidingWindowFeature:
        ...
        for chunk in chunks:
            ...
            # --- FIX: Calculate start index using Time / Step ---
            # chunk.start is in seconds. rf_step is seconds/frame.
            # This is robust and consistent with how output windows are defined.
            ...
            # Ensure we don't write before index 0 or after array end
            ...
            # Clip end frame if it exceeds buffer
            if end_frame_global > num_frames:
                valid_len = num_frames - start_frame_global
                if valid_len <= 0:
                    continue
                # Slice prediction to match remaining buffer space

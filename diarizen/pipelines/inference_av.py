...
from pyannote.core import SlidingWindow, SlidingWindowFeature, Segment, Annotation
...
class AV_DiariZenPipeline(DiariZenPipeline):
    ...
    def get_av_segmentations(self, waveform: torch.Tensor, sample_rate: int, sess_name: str) -> SlidingWindowFeature:
        ...
        for chunk in chunks:
            ...
            # Ensure fixed chunk size for batching/stacking
            expected_end_sample = start_sample + int(chunk_duration * sample_rate)
            
            wav_chunk = waveform[:, start_sample:expected_end_sample]
            
            # Pad Audio if short (end of file)
            ...
            with torch.no_grad():
                # Shape: (1, frames, powerset_classes) if powerset=True
                prediction = model(inputs)
                
                # --- FIX: Handle Powerset Conversion ---
                if model.specifications.powerset:
                    # 1. Convert Log-Probs (LogSoftmax) to Probs (0-1)
                    prediction = prediction.exp()
                    # 2. Convert Powerset to Multilabel (Speakers)
                    prediction = model.powerset.to_multilabel(prediction)
                # ---------------------------------------
            
            # Remove batch dim: (frames, speakers)
            predictions.append(prediction[0].cpu().numpy())

        # Stack into 3D tensor: (num_chunks, frames, speakers)
        if not predictions:
    ...
    def __call__(self, in_wav, sess_name=None):
        ...
        if len(embeddings) == 0:
            print(f"Warning: No embeddings extracted for {sess_name}. Returning empty annotation.")

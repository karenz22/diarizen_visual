...
class AV_DiariZenPipeline(DiariZenPipeline):
    ...
    def get_av_segmentations(self, waveform: torch.Tensor, sample_rate: int, sess_name: str) -> SlidingWindowFeature:
        """
        Returns stacked segmentation scores for each chunk.
        ...
        for chunk in chunks:
            ...
            # Handle padding if chunk is smaller than expected (end of file)
            # This ensures every prediction has the same shape
    ...
    def __call__(self, in_wav, sess_name=None):
        ...
        # 3. Rest of pipeline handles aggregation implicitly
        ...
        # Inactive speakers check works on aggregated data, but here segmentations is chunked.
        # Pyannote pipeline handles `reconstruct` which aggregates chunks and clusters.
        
        # NOTE: inactive_speakers check in original script summed axis=1 (time). 
        # For chunked data (N, T, C), sum(axis=1) gives (N, C) -> activity per chunk.
        # This is correct for the logic below.

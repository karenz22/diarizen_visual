...
class AV_DiariZenPipeline(DiariZenPipeline):
    ...
    def get_av_segmentations(self, waveform: torch.Tensor, sample_rate: int, sess_name: str) -> SlidingWindowFeature:
        """
        Returns stacked 3D segmentation scores for each chunk.
        Output Shape: (num_chunks, frames_per_chunk, num_classes)
        """
        ...
        # Sliding window for inputs (audio chunks)
        ...
        for chunk in chunks:
            # We enforce a FIXED chunk duration to ensure all output tensors have the same shape.
            # Even if the audio ends early, we pad it to the full chunk_duration.
            
            start_sample = int(chunk.start * sample_rate)
            expected_end_sample = int((chunk.start + chunk_duration) * sample_rate)
            
            # Slicing safely (handles end of file)
            wav_chunk = waveform[:, start_sample:expected_end_sample]
            ...
            # Load Visual (Request full duration to match audio padding logic)
            # This prevents stretching/interpolation artifacts in the model
            visual_chunk = self.load_visual_chunk(sess_name, chunk.start, chunk.start + chunk_duration)
            visual_chunk = visual_chunk.unsqueeze(0).to(device) 
            
            inputs = {"waveforms": wav_chunk, "visual": visual_chunk}
            with torch.no_grad():
                # Shape: (1, frames, classes)
                prediction = model(inputs)
            
            # Remove batch dim: (frames, classes)
            predictions.append(prediction[0].cpu().numpy())

        # Stack into 3D tensor: (num_chunks, frames, classes)
        if not predictions:
             # Return empty feature if no audio
             return SlidingWindowFeature(np.zeros((0, 0, 0)), sliding_window)

        data = np.stack(predictions)
        
        # Returns 3D features. sliding_window describes the CHUNKS.
    ...
    def __call__(self, in_wav, sess_name=None):
        ...
        # 1. Get Chunked Segmentations (Rank 3)
        segmentations = self.get_av_segmentations(waveform, sample_rate, sess_name)

        # 2. Apply Median Filtering (Expects Rank 3)
        if self.apply_median_filtering:
            # size=(batch/chunk, time, classes) -> (1, 11, 1) filters along time
            segmentations.data = median_filter(segmentations.data, size=(1, 11, 1), mode='reflect')

        binarized_segmentations = segmentations

        # 3. Speaker Count with SlidingWindow object
        _, rf_duration, rf_step = self._segmentation.model.get_rf_info
        
        # This SlidingWindow describes the internal frame resolution of the model output
        # It is required by Inference.aggregate to map the (Frames) dim to time.
        frames_resolution = SlidingWindow(start=0.0, duration=rf_duration, step=rf_step)

        count = self.speaker_count(
            binarized_segmentations,
            frames_resolution, # Passing SlidingWindow object (FIX for AttributeError)
            warm_up=(0.0, 0.0),
        )
        ...
        # Handle inactive speakers (summing along frames axis 1, resulting in (N_chunks, N_classes))

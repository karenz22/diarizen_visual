...
class AV_DiariZenPipeline(DiariZenPipeline):
    ...
    def load_visual_chunk(self, session: str, start: float, end: float) -> torch.Tensor:
        ...
        if os.path.exists(video_path):
            ...
            print(f"Warning: Video file {video_path} not found. Using black frames.")
    ...
    def get_av_segmentations(self, waveform: torch.Tensor, sample_rate: int, sess_name: str) -> SlidingWindowFeature:
        """
        Runs inference and performs Overlap-Add aggregation.
        Returns a 2D SlidingWindowFeature: (Total_Frames, Num_Classes)
        """
        ...
        # Get Model Output Resolution (rf_step is seconds per frame)
        _, rf_duration, rf_step = model.get_rf_info
        
        # Calculate total output frames for the whole file
        ...
        # 2D Aggregation buffers: (Time, Classes)
        ...
        for chunk in chunks:
            ...
            if start_frame_global < 0: 
                # Slice prediction if it starts before 0 (unlikely with this logic, but safe)
                prediction = prediction[-start_frame_global:]
    ...
    def __call__(self, in_wav, sess_name=None):
        ...
        # 1. Get Aggregated 2D Segmentations
        segmentations = self.get_av_segmentations(waveform, sample_rate, sess_name)

        # 2. Apply Median Filtering (2D)
        if self.apply_median_filtering:
            # FIX 1: Filter size (11, 1) for (Time, Classes)
            segmentations.data = median_filter(segmentations.data, size=(11, 1), mode='reflect')

        binarized_segmentations = segmentations

        # FIX 2: Create SlidingWindow object for speaker_count
        ...
        count = self.speaker_count(
            ...
            frames_resolution, # Pass SlidingWindow object
            warm_up=(0.0, 0.0),
        )
        ...
        # Note: reconstruction expects hard_clusters aligned with segments
        # If clustering returns fewer clusters than original segments due to silence,
        # we need to be careful. However, pyannote usually aligns this.
        
        # In this specific pipeline script logic, 'hard_clusters' are assigned to active chunks.
        # But 'binarized_segmentations' here is full-length 2D.
        # The base Pipeline.clustering implementation typically re-segments the input based on 'segmentations'
        # and returns clusters. 
        
        # NOTE: standard Pyannote SpeakerDiarization.clustering works on 'embeddings' which are derived 
        # from specific sliding windows over 'segmentations'.

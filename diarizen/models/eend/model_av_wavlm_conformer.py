...
class AVModel(BaseModel):
    ...
    def load_wavlm(self, source: str, ckpt_path: str = None):
        """
        Load WavLM model with prefix handling.
        """
        config = None
        state_dict = None
        ...
        if ckpt_path is not None and os.path.isfile(ckpt_path):
            ...
            # --- FIX: Handle 'wavlm_model.' prefix mismatch ---
            new_weights = {}
            for k, v in weights.items():
                if k.startswith("wavlm_model."):
                    # Strip 'wavlm_model.' prefix
                    new_k = k[len("wavlm_model."):] 
                    new_weights[new_k] = v
                elif k.startswith("module.wavlm_model."):
                    # Strip 'module.wavlm_model.' prefix (DDP case)
                    new_k = k[len("module.wavlm_model."):]
                    new_weights[new_k] = v
                else:
                    new_weights[k] = v
            ...
            if missing:
                # Filter out trivial missing keys if any (often related to mask_emb which is fine)
                missing = [m for m in missing if "mask_emb" not in m]
                if missing:
